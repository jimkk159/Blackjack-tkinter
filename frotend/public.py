...
def toggle_color_bg(item, color_a, color_b):
...
def start_blink(window, canvas, item, delay_time, with_bg=False):
    global blink_id
    blink_id = None
    blink(window, canvas, item, delay_time, with_bg)
...
def blink(window, canvas, item, delay_time, with_bg):
    global blink_id
    toggle_color_fg(canvas, item, "black", WHITE)

    if with_bg:
        toggle_color_bg(item, WHITE, "black")
    blink_id = window.after(delay_time, blink, window, canvas, item, delay_time, with_bg)
